...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
            ...
            for(int i = 0; i < 12; i++){
                drywetProbability[i] = drywet[row][2+i];
                wetwetProbability[i] = wetwet[row][2+i];
            }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
            ...
            random = StdRandom.uniform(0.0, 1.0);    
                if(forecast[i - 1] == WET){
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        double d = drywetProbability[month];
        double w = wetwetProbability[month];

        int[] forecast = forecastGenerator(d, w, numberOfDaysInMonth[month]);
        return forecast;
    ...
    public static int bestWeekToTravel(int[] forecast){
        int find = lengthOfLongestSpell(forecast, DRY);
        String a = "";
        String b = "";
        for(int i = 0; i < forecast.length;i++){
            a = a + forecast[i];
